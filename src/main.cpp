...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight <= last_pow_block && block.IsProofOfStake())
        return state.DoS(100, error("ConnectBlock() : PoS period not active %d", pindex->nHeight),
        ...
        BOOST_REVERSE_FOREACH (CBlockIndex* pindexConnect, vpindexToConnect) {
            if (!ConnectTip(state, pindexConnect, pindexConnect == pindexMostWork ? pblock : NULL, fAlreadyChecked)) {
                if (state.IsInvalid()) {
                    ...
                    if (!state.CorruptionPossible())// && vpindexToConnect.back()->nHeight > 950000)
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (block.GetBlockTime() > Params().GetConsensus().ZC_TimeStart) {
        if(block.nVersion < 4) {
            //strprintf("block time %u, zc time %u",block.GetBlockTime(), Params().GetConsensus().ZC_TimeStart);
            //printf(Params().GetConsensus().ZC_TimeStart);
            return state.DoS(50, error("CheckBlockHeader() : block version must be above 4 after ZerocoinStartHeight %d, z: %d", block.GetBlockTime(), Params().GetConsensus().ZC_TimeStart),
            REJECT_INVALID, "block-version");
        }
    } else {
        if (block.nVersion >= 4 && !sporkManager.IsSporkActive(SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2))
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        if (tx.HasZerocoinSpendInputs() && fZerocoinActive) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
