// Copyright (c) 2017-2020 The BirakeCoin developers
    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zBIR Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < nRequiredConfs)
                ...
                strReason = "Your wallet is locked. Impossible to spend zBIR.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBIR seed used to mint this zPIV is not the same as currently hold in the wallet";
            else
