// Copyright (c) 2019-2020 The BirakeCoin developers
    ...
    ui(new Ui::PrivacyWidget)
{
    ...
    // Only Convert to BIR enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzBIR->setTitleClassAndText("btn-title-grey", tr("Total 0 zBIR"));
    ui->btnTotalzBIR->setSubTitleClassAndText("text-subtitle", tr("Show denominations of z owned."));
    ui->btnTotalzBIR->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", tr("Coin Control"));
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", tr("Select BIR outputs to mint into z."));
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", tr("Reset Spent zBIR"));
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", tr("Reset zerocoin database."));

    connect(ui->btnTotalzBIR, &OptionButton::clicked, this, &PrivacyWidget::onTotalZpivClicked);
...
void PrivacyWidget::onMintSelected(bool isMint)
{
    QString btnText;
    if (isMint) {
        btnText = tr("Mint zBIR");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of BIR to mint into zBIR"));
    } else {
        btnText = tr("Convert back to BIR");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of z to convert back into BIR"));
    }
...
void PrivacyWidget::onTotalZpivClicked()
{
    ...
    if (!isVisible) {
        ...
        ui->btnTotalzBIR->setRightIconClass("btn-dropdown", true);
    } else {
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzBIR->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zBIR is currently undergoing maintenance"));
        return;
    }
    ...
    if (!ctx.isValid()) {
        inform(tr("You need to unlock the wallet to be able to %1 zBIR").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value)
{
    ...
    if (!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)) {
        ...
        // Mint succeed
        inform(tr("zBIR minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value)
{
    ...
    if (!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zBIR converted back to BIR"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked()
{
    if (ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any BIR to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply()
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBIR </b>";
    ...
    ui->btnTotalzBIR->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
